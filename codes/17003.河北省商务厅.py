...
def get_content(start_date=now):
    url_index = 'http://swt.hebei.gov.cn/newsContentform/infoPublicList?d-16399-p=1&y=1&channelId=621'
    ...
    while page_turning:
        ...
        for i in range(0, 30, 3):
            ...
            if date < start_date:
                ...
                page_turning = False
                break
            title = text.find('a').get_text().strip().replace('·', '').replace('\n', '')  # 获取纯文本内容（不带html标签）
            if re.search(title_pattern, title):  # 匹配标签关键字
                logger.info(f"{title}\t{date}")
                if text.find('a').get('href').startswith('http'):
                    href = text.find('a').get('href')
                else:
                    href = "http://swt.hebei.gov.cn" + text.find('a').get('href')
    ...
    for index, url_detail in enumerate(url_list):
        ...
        if match_res:
            ...
            result += f"{GOVERMENT}\t{','.join(['的通知'])}\t{title_list[index]}\t{date_list[index]}\t{url_detail}\n"
