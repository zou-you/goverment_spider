...
def get_content(start_date=now):
    url_index = "https://sw.beijing.gov.cn/tzgg/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.newslist a')
        time_list = bs.select('.newslist span')

        # 找出符合要求的时间以及标题
        for text, tim in zip(text_list, time_list):
            date = tim.get_text(strip=True).replace('[', '').replace(']', '')
            if date < start_date:
            ...
            if not href.startswith('http'):
                href = 'https://sw.beijing.gov.cn/tzgg/' + href.replace('./', '', 1)
    ...
    for index, url_detail in enumerate(url_list):
        ...
        title = bs.find('h3').get_text(strip=True).replace('·', '').replace('\n', '')
