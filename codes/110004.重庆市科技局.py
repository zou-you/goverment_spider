...
def get_content(start_date=now):
    url_index = "https://kjj.cq.gov.cn/zwxx_176/tzgg/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".tab-item li.clearfix a[title]")
        time_list = bs.select('.tab-item li.clearfix span')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://kjj.cq.gov.cn/zwxx_176/tzgg/" + text['href'])
                date_list.append(date)
                title_list.append(title)

        # 进行翻页查找
        if page_num != 20:
            url_index = f"https://kjj.cq.gov.cn/zwxx_176/tzgg/index_{page_num}.html"
            page_num += 1
            # 随机睡眠
