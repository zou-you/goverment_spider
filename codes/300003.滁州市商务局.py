...
def get_content(start_date=now):
    url_index = "https://swj.chuzhou.gov.cn/xwdt/tzgg/index.html"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".doc_list.list-5092274.list2-5092274 li a")
        time_list = bs.select(".doc_list.list-5092274.list2-5092274 li span.right.date")
        ...
        if page_num != 10:
            page_num += 1
            url_index = f"https://swj.chuzhou.gov.cn/content/column/5092274?pageIndex={page_num}"
            # 随机睡眠
