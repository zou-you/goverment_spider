...
def get_content(start_date=now):
    url_index = "http://gxj.xinxiang.gov.cn/zhengcewenjian.html"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".arc-item a")
        time_list = bs.select(".arc-item span")
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("http://gxj.xinxiang.gov.cn/" + text['href'])
        ...
        if page_num != 20:
            page_num += 1
            url_index = f"http://gxj.xinxiang.gov.cn/zhengcewenjian.html?page={page_num}"
            # 随机睡眠
