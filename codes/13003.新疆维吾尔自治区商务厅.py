...
def get_content(start_date=now):
    url_index = "http://swt.xinjiang.gov.cn/xjswt/c114330/list.shtml"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.commonlist-right-ul li a')
        time_list = bs.select('.commonlist-right-ul li span')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("http://swt.xinjiang.gov.cn" + text['href'])
