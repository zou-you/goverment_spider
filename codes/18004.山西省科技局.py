...
def get_content(start_date=now):
    url_index = "https://kjt.shanxi.gov.cn/xxgk/zfxxgkml/azt_11688/tzgg_11716/"
    ...
    # 定义正则表达式
    record_pattern = r'<td><a href="(.*?)" target="_blank".*?">(.*?)</a></td>.*?(\d{4}.\d{2}.\d{2})'
        ...
        for record in records:
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                if not href.startswith('http'):
                    href = "https://kjt.shanxi.gov.cn/xxgk/zfxxgkml/" + href.replace('../../', '', 1)
