...
def get_content(start_date=now):
    url_index = 'https://jx.ah.gov.cn/sy/wjgg/index.html'
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".doc_list.list-51520031 li a")
        time_list = bs.select(".doc_list.list-51520031 li span.right.date")
        ...
        if page_num != 20:
            page_num += 1
            url_index = f"https://jx.ah.gov.cn/content/column/51520031?SearchWords=&pageIndex={page_num}"
            # 随机睡眠
