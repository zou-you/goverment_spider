...
def get_content(start_date=now):
    url_index = "http://fgw.xinxiang.gov.cn/leibiao/list.aspx?lcid=52"
    ...
    while page_turning:
        ...
        for bs_ in bs_list:
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("http://fgw.xinxiang.gov.cn/" + bs_['href'])
        ...
        if page_num != 10:
            page_num += 1
            url_index = f"http://fgw.xinxiang.gov.cn/leibiao/list.aspx?lcid=52&page={page_num}"
            # 随机睡眠
