...
def get_content(start_date=now):
    url_index = "https://gxt.henan.gov.cn/xxgk/wjjb/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".zfxxgk_zdgkc li a")
        time_list = bs.select(".zfxxgk_zdgkc li b")
        ...
        # 进行翻页查找
        if page_num != 5:
            url_index = f"https://gxt.henan.gov.cn/xxgk/wjjb/index_{page_num}.html"
            page_num += 1
            # 随机睡眠
