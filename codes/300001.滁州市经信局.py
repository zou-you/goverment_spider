...
def get_content(start_date=now):
    url_index = "https://jxj.chuzhou.gov.cn/xwdt/tzgg/index.html"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".doc_list.list-4014759.list2-4014759 li a")
        time_list = bs.select(".doc_list.list-4014759.list2-4014759 li span.right.date")
        ...
        if page_num != 10:
            page_num += 1
            url_index = f"https://jxj.chuzhou.gov.cn/content/column/4014759?pageIndex={page_num}"
            # 随机睡眠
