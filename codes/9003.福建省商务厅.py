...
def get_content(start_date=now):
    url_index = "https://swt.fujian.gov.cn/xxgk/tzgg/"
    ...
    while page_turning:
        ...
        bs_list = bs.select('.gl-ul.clearflx li>a[title]')

        # 找出符合要求的时间以及标题
        for item  in bs_list:
            news_num += 1
            # 提取 <a> 标签中的 href 和 title
            href = item['href'].replace('..', '') # 获取链接
            title = item['title'] # 获取标题

            # 提取时间信息
            date = item.select('.fr.li-btn')[0].get_text(strip=True)
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://swt.fujian.gov.cn/xxgk" + href)
