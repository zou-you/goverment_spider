...
def get_content(start_date=now):
    url_index = "https://kjt.sc.gov.cn/kjt/gstz/newschild.shtml"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".column-list ul li a")
        time_list = bs.select(".column-list ul li span")
        ...
        if page_num != 15:
            page_num += 1
            url_index = f"https://kjt.sc.gov.cn/kjt/gstz/newschild_{page_num}.shtml"
            # 随机睡眠
