...
def get_content(start_date=now):
    url_index = "http://jcswj.jcgov.gov.cn/zwxx_1/tzgg/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('li p.text a')
        time_list = bs.select('.time')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                if not href.startswith('http'):
                    href = 'http://jcswj.jcgov.gov.cn/zwxx_1/tzgg/' + href.replace('./', '', 1)
