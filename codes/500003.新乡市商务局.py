...
def get_content(start_date=now):
    url_index = "https://commerce.xinxiang.gov.cn/news/15"
    ...
    while page_turning:
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://commerce.xinxiang.gov.cn/" + text['href'])
        ...
        if page_num != 10:
            page_num += 1
            url_index = f"https://commerce.xinxiang.gov.cn/news/news_type/type/15/p/{page_num}"
            # 随机睡眠
