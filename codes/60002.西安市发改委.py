...
def get_content(start_date=now):
    url_index = "http://xadrc.xa.gov.cn/xwzx/tzgg/1.html"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".title a[alt]")
        time_list = bs.select(".news-date.col-sm-3.col-md-3.col-lg-2")
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("http://xadrc.xa.gov.cn/" + text['href'])
        ...
        if page_num != 20:
            page_num += 1
            url_index = f"http://xadrc.xa.gov.cn/xwzx/tzgg/{page_num}.html"
            # 随机睡眠
