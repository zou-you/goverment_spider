...
def get_content(start_date=now):
    url_index = "https://dofcom.hainan.gov.cn/dofcom/0400/list.shtml?ddtab=true"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.flfg_038-01 a')
        time_list = bs.select('.flfg_038-01 em')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://dofcom.hainan.gov.cn" + text['href'])
