...
def get_content(start_date=now):
    url_index = 'http://www.sasac.gov.cn/n2588035/n2588320/n2588335/index.html'
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".zsy_conlist ul span li a")
        time_list = bs.select(".zsy_conlist ul span li span")
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("http://www.sasac.gov.cn/" + text['href'])
