...
def get_content(start_date=now):
    url_index = "https://kjt.zj.gov.cn/col/col1229225203/index.html"
    ...
    # 定义正则表达式
    record_pattern = r'<li>\s*<a href="(.*?)" title="(.*?)" target="_blank">.*?</a>.*?<span>(.*?)</span>'
    ...
    while page_turning:
        ...
        for record in records:
            ...
            title = title.strip().replace('<br>', '').replace('<br/>', '').replace('<br/>', '').replace('\r','')
            if date < start_date:
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://kjt.zj.gov.cn/" + href)
