...
def get_content(start_date=now):
    url_index = "https://fzggw.cq.gov.cn/zwxx/tzgg/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".clearfix a[title]")
        time_list = bs.select('.clearfix span.rt')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://fzggw.cq.gov.cn/zwxx/tzgg/" + text['href'])
                date_list.append(date)
                title_list.append(title)

        # 进行翻页查找
        if page_num != 10:
            url_index = f"https://fzggw.cq.gov.cn/zwxx/tzgg/index_{page_num}.html"
            page_num += 1
            # 随机睡眠
