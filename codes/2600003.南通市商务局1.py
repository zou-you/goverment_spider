...
def get_content(start_date=now):
    url_index = "http://swj.nantong.gov.cn/ntsswj/gggs/gggs.html#"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.list-ul li a')
        time_list = bs.select('.list-ul li span')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                if not href.startswith('http'):
                    href = 'http://swj.nantong.gov.cn' + href
