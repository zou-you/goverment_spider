...
def get_content(start_date=now):
    url_index = "http://swt.qinghai.gov.cn/sy/tzgg/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.somelist .clearfix a')
        time_list = bs.select('.somelist .clearfix span')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                if not href.startswith('http'):
                    href = 'http://swt.qinghai.gov.cn/sy/tzgg/' + href.replace('./', '', 1)
