...
def get_content(start_date=now):
    url_index = "https://sw.beijing.gov.cn/zwxx/2024zcwj/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.zcwj_right li a')
        time_list = bs.select('.zcwj_right li span')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                if not href.startswith('http'):
                    href = 'https://sw.beijing.gov.cn/zwxx/2024zcwj/' + href.replace('./', '', 1)
