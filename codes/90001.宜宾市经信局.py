...
def get_content(start_date=now):
    ...
    url_index = 'https://jjhxxhj.yibin.gov.cn/sy/wjtz/'
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".list_list a")
        time_list = bs.select(".list_list span")
        ...
        for text, tim in zip(text_list, time_list):
            ...
            url_list.append("https://jjhxxhj.yibin.gov.cn/sy/wjtz/" + text['href'])
            date_list.append(date)

        # 进行翻页查找
        if page_num != 10:
            url_index = f"https://jjhxxhj.yibin.gov.cn/sy/wjtz/index_{page_num}.html"
            page_num += 1
            # 随机睡眠
    ...
    for index, url_detail in enumerate(url_list):
        ...
        title = bs.find('td', class_='content_ti').get_text(strip=True).strip().replace('·', '').replace('\n', '')
