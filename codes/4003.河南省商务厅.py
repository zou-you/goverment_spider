...
def get_content(start_date=now):
    url_index = "https://hnsswt.henan.gov.cn/zwgk/tzgg/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".zfxxgk_zdgkc ul li a")
        time_list = bs.select(".zfxxgk_zdgkc ul li b")
        ...
        # 进行翻页查找
        if page_num != 10:
            url_index = f"https://hnsswt.henan.gov.cn/zwgk/tzgg/index_{page_num}.html"
            page_num += 1
            # 随机睡眠
