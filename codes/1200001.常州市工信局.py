...
def get_content(start_date=now):
    url_index = "https://www.changzhou.gov.cn/gova/tzgg.php?catname=jxw_02_04"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.container.ty.jb table>tbody>tr>td>a[target="_blank"]')
        time_list = bs.select('.container.ty.jb table>tbody>tr>td[align="center"]')
        time_list = [time_list[i + 3] for i in range(0, len(time_list), 4)]
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://www.changzhou.gov.cn/" + text['href'])
        ...
        if page_num != 20:
            page_num += 1
            url_index = f"https://www.changzhou.gov.cn/gova/tzgg.php?catname=jxw_02_04&page={page_num}"
            # 随机睡眠
    ...
    for index, url_detail in enumerate(url_list):
        ...
        fz_date = bs.find(width="298").get_text().split("废止日期：")[-1]  # 注意通告废止日期
        if len(fz_date) == 10 and fz_date < now:
            continue
