...
def get_content(start_date=now):
    url_index = "http://swj.ningbo.gov.cn/col/col1229051920/index.html"
    ...
    # 定义正则表达式
    record_pattern = r'<a class="ellipsis" href="(.*?)" title="(.*?)">.*?</a><span class="date">\[(\d{4}-\d{2}-\d{2})\]</span>'
    ...
    while page_turning:
        ...
        for record in records:
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("http://swj.ningbo.gov.cn" + href)
