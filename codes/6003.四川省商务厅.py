...
def get_content(start_date=now):
    url_index = "https://swt.sc.gov.cn/sccom/tzgg/list.shtml"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('li [title]')
        time_list = bs.select('li [style="float:right"]')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://swt.sc.gov.cn/" + text['href'])
        ...
        if page_num != 15:
            page_num += 1
            url_index = f"https://swt.sc.gov.cn/sccom/tzgg/list_{page_num}.shtml"
            # 随机睡眠
