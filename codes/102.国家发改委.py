...
def get_content(start_date=now):
    url_index = 'https://www.ndrc.gov.cn/xwdt/tzgg/index.html'
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.u-list li a')
        time_list = bs.select(".u-list li span")

        # 找出符合要求的时间以及标题
        for text, tim in zip(text_list, time_list):
            date = tim.get_text(strip=True).replace('/', '-')
            if date < start_date:
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://www.ndrc.gov.cn/xwdt/tzgg/" + text['href'])
                date_list.append(date)
                title_list.append(title)

        # 进行翻页查找
        if page_num != 20:
            url_index = f'https://www.ndrc.gov.cn/xwdt/tzgg/index_{page_num}.html'
            page_num += 1
            # 随机睡眠
