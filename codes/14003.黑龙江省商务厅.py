...
def get_content(start_date=now):
    url_index = "https://sswt.hlj.gov.cn/sswt/c103658/list.shtml"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.listul li a')
        time_list = bs.select('.listul li span')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://sswt.hlj.gov.cn" + text['href'])
