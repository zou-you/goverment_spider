...
def get_content(start_date=now):
    ...
    url_index = "https://swt.ln.gov.cn/swt/ywxx/tzgg/index.shtml"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".l-gl-main.cl li a")
        time_list = bs.select(".l-gl-main-num")
        ...
        for text, tim in zip(text_list, time_list):
            ...
            url_list.append("https://swt.ln.gov.cn" + text['href'])
    ...
    for index, url_detail in enumerate(url_list):
        ...
        title = bs.find('h2', class_='d_title').get_text(strip=True).strip().replace('·', '').replace('\n', '')
