...
def get_content(start_date=now):
    url_index = "http://srsw.zgsr.gov.cn/swj/gsgg/zwgk_xxgklist.shtml"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.zcwj-wj a[href]')
        time_list = bs.select('.span_date')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                if not href.startswith('http'):
                    href = 'http://srsw.zgsr.gov.cn' + href
