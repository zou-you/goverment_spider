...
def get_content(start_date=now):
    url_index = "http://jxj.ningbo.gov.cn/col/col1229561613/index.html"
    ...
    # 定义正则表达式
    record_pattern = r'<a target="_blank" href="(.*?)" >(.*?)</a><span>.*?(\d{4}-\d{2}-\d{2})'
    ...
    while page_turning:
        ...
        for record in records:
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("http://jxj.ningbo.gov.cn" + href)
