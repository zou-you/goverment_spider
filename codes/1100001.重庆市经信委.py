...
def get_content(start_date=now):
    url_index = "https://jjxxw.cq.gov.cn/zwgk_213/zcwj/qtwj/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        bs_list = bs.select(".li-4 a")

        # 找出符合要求的时间以及标题
        for bs_ in bs_list:
            tim, text = bs_.select('span')[0], bs_.select('p')[0]
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://jjxxw.cq.gov.cn/zwgk_213/zcwj/qtwj/" + bs_['href'])
                date_list.append(date)
                title_list.append(title)

        # 进行翻页查找
        if page_num != 20:
            url_index = f"https://jjxxw.cq.gov.cn/zwgk_213/zcwj/qtwj/index_{page_num}.html"
            page_num += 1
            # 随机睡眠
