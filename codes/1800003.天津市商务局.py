...
def get_content(start_date=now):
    url_index = "https://shangwuju.tj.gov.cn/tjsswjzz/zwgk/zcfg_48995/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.list a')
        time_list = bs.select('.list span')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if not href.startswith('http'):
                href = 'https://shangwuju.tj.gov.cn/tjsswjzz/zwgk/zcfg_48995/' + href.replace('./', '', 1)
    ...
    for index, url_detail in enumerate(url_list):
        ...
        title = bs.find('div', class_='trs_editor_view TRS_UEDITOR trs_paper_default trs_word')
        if not title:
            title = bs.find('div', class_='trs_editor_view TRS_UEDITOR trs_paper_default trs_word trs_web')
        if not title:
            continue
        title = title.find('p').get_text(strip=True).replace('·', '').replace('\n', '')
