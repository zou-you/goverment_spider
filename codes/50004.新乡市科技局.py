...
def get_content(start_date=now):
    url_index = "https://kjj.xinxiang.gov.cn/news/9"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".news-item.d-flex.justify-content-between.align-items-center")
        time_list = bs.select(".news-time.d-none.d-lg-inline-block")
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://kjj.xinxiang.gov.cn/" + text['href'])
        ...
        if page_num != 10:
            page_num += 1
            url_index = f"https://kjj.xinxiang.gov.cn/news/news_type/type/9/p/{page_num}"
            # 随机睡眠
