...
def get_content(start_date=now):
    url_index = "http://kjt.ah.gov.cn/kjzx/tzgg/index.html"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".doc_list.list-26701351 li a")
        time_list = bs.select(".doc_list.list-26701351 li span.right.date")
        ...
        if page_num != 20:
            page_num += 1
            url_index = f"http://kjt.ah.gov.cn/content/column/26701351?pageIndex={page_num}"
            # 随机睡眠
