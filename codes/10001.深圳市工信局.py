...
def get_content(start_date=now):
    url_index = 'http://gxj.sz.gov.cn/xxgk/xxgkml/qt/tzgg/'
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.Listcont a')
        time_list = bs.select(".Listcont span")
        ...
        if page_num != 40:
            page_num += 1
            url_index = f"http://gxj.sz.gov.cn/xxgk/xxgkml/qt/tzgg/index_{page_num}.html"
            # 随机睡眠
