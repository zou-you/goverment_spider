...
def get_content(start_date=now):
    url_index = "https://kjt.qinghai.gov.cn/content/lists/cid/7/pid/26/page/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.list_ul li a')
        time_list = bs.select('.list_ul li span')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if date < start_date:
                ...
                page_turning = False
                break
            title = text.get_text().strip().split(' ')[0]
