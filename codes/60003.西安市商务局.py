...
def get_content(start_date=now):
    url_index = "http://xasswj.xa.gov.cn/xwzx/tzgg/1.html"
    ...
    while page_turning:
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("http://xasswj.xa.gov.cn/" + text['href'])
        ...
        if page_num != 10:
            page_num += 1
            url_index = f"http://xasswj.xa.gov.cn/xwzx/tzgg/{page_num}.html"
            # 随机睡眠
