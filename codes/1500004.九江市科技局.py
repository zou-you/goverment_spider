...
def get_content(start_date=now):
    ...
    url_index = "https://kjj.jiujiang.gov.cn/zwzx_253/gggs/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.r_box a[title]')
        time_list = bs.select('.bd span')
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_ = text['href'] if "http" in text['href'] else "https://kjj.jiujiang.gov.cn/zwzx_253/gggs/" + text['href']
        ...
        # 进行翻页查找
        if page_num != 10:
            url_index = f"https://kjj.jiujiang.gov.cn/zwzx_253/gggs/index_{page_num}.html"
            page_num += 1
