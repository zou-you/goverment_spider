...
def get_content(start_date=now):
    url_index = 'http://www.mofcom.gov.cn/article/gztz/'
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select('.txtList_01 li a')
        time_list = bs.select(".txtList_01 li span")
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("http://www.mofcom.gov.cn/" + text['href'])
        ...
        if page_num != 5:
            page_num += 1
            url_index = f"http://www.mofcom.gov.cn/article/gztz/?{page_num}"
            # 随机睡眠
