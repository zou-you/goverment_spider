...
def get_content(start_date=now):
    ...
    while page_turning:
        ...
        url_index = f"http://www.jiangsu.gov.cn/jrobot/search.do?webid=23&analyzeType=1&pg=30&p={page_num}&tpl=2&category=xwzx&q=&pos=title%2Ccontent%2C_default_search&od=1&date=&date="

        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".jsearch-result-title a")
        time_list = bs.select(".jsearch-result-date")

        # 找出符合要求的时间以及标题
        for text, tim in zip(text_list, time_list):
            date = tim.get_text(strip=True).replace("年", "-").replace("月", "-").replace("日", "")
            if date < start_date:
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("http://www.jiangsu.gov.cn/" + text['href'])
