...
def get_content(start_date=now):
    ...
    url_index = "https://fgw.henan.gov.cn/xwzx/tzgg/"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".news-list ul li a")
        time_list = bs.select(".news-list ul li span")
        ...
        # 进行翻页查找
        if page_num != 5:
            url_index = f"https://fgw.henan.gov.cn/xwzx/tzgg/index_{page_num}.html"
            page_num += 1
            # 随机睡眠
    ...
    for index, url_detail in enumerate(url_list):
        ...
        title = bs.find('p', class_='yTit').get_text(strip=True).strip().replace('·', '').replace('\n', '')
