...
def get_content(start_date=now):
    url_index = "https://jxt.sc.gov.cn/scjxt/wjfb/common_list.shtml"
    ...
    while page_turning:
        ...
        # 访问链接
        bs = get_bs(url_index)
        text_list = bs.select(".f-20 div a")
        time_list = bs.select(".f-20 span.list-time.f-20.fr")
        ...
        for text, tim in zip(text_list, time_list):
            ...
            if re.search(title_pattern, title):  # 匹配标签关键字
                ...
                url_list.append("https://jxt.sc.gov.cn/" + text['href'])
        ...
        if page_num != 20:
            page_num += 1
            url_index = f"https://jxt.sc.gov.cn/scjxt/wjfb/common_list_{page_num}.shtml"
            # 随机睡眠
